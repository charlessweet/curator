...
class LinkToFacebookCard extends React.Component{
	...
	render(){
		let loginActionDiv = <div>You are not logged in. You must log in to Facebook to import your articles from biaschecker.org.<br/><br/><Button className='primary' onClick={this.handleLogin}>Login to Facebook</Button></div>
		if(this.state.loggedInToFacebook){
			...
			<Button className='primary' onClick={this.handleSubmit}>Import Articles</Button>
			</div>
		}
		return <Card>
					<CardContent>
